...
\end{verbatim}
Note that \code{FrameLengths} and \code{MidFTimes} are both column
...
\verb|ntrapz|.  \verb|ntrapz| currently has exactly the same
...
\item \code{micreateimage} - Create the MIimage variable in an
...
vectors.  Note that the indexing of \code{start\_vector} and
\code{stop\_vector} is zero-based.
\begin{verbatim}
